...
<Step title="Select the blockchain">
```bash
...
<Step title="Select events to index">
```bash
...
<Step title="Finish or add more contracts">
...
<Step title="Specify ABI JSON file">
...
By default, all selected events are included, but you can manually adjust the file if needed. See the detailed guide on [configuration file](/HyperIndex/Guides/configuration-file).
...
See more details in the [schema file guide](/HyperIndex/Guides/schema-file).
...
Learn more in the [event handlers guide](/HyperIndex/Guides/event-handlers).

---

**Congratulations!** Your HyperIndex indexer is now ready to run and query data!

Next step: [Running your Indexer locally](/HyperIndex/Guides/running-locally) or [Deploying to Hosted Service](/HyperIndex/Hosted_Service/hosted-service).
