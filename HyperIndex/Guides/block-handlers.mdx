...
<Note>
**New in v2.29** - Run logic on every block or at an interval.
</Note>
...
<Note>
Block Handlers require [Preload Optimization](/HyperIndex/Advanced/preload-optimization) to be enabled. Make sure to enable it in your `config.yaml` file. And don't forget that it makes your handlers run twice.
</Note>
...
- `block` - The block object.
  ...
  - More fields will be added in the future. Let us know in [Discord](https://discord.gg/envio) if you need any specific fields. You can also use [Effect API](/HyperIndex/Advanced/effect-api) to get the data from RPC.
- `context` - Exactly the same as the Event Handlers [Context Object](/HyperIndex/Guides/event-handlers#context-object).
...
Here's the recipe for how to speed up your historical sync by increasing the interval for historical blocks.
...
<Tip>
We recommend exploring this approach together with [HyperSync](/HyperSync/overview) client to effectively query data for big block ranges.
</Tip>
...
- Block Handlers require [Preload Optimization](/HyperIndex/Advanced/preload-optimization) to be enabled.
- The [Ordered Multichain mode](/HyperIndex/Advanced/multichain-indexing#ordered-mode) is not supported.
- Only EVM chains are supported. Currently no support for [Fuel](/HyperIndex/fuel/fuel) chains.
- No [test framework](/HyperIndex/Guides/testing) support.
