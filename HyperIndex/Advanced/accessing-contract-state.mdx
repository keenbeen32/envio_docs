...
3. Learn about [Preload Optimisation](/HyperIndex/Advanced/preload-optimization) and how it makes your indexer thousands of times faster
4. Use [Effect API](/HyperIndex/Advanced/effect-api) with built-in caching and Viem transport level batching
...
```yaml icon="/static/icons/yaml-icon.png"
...
<Step title="Setup the Indexer Configuration">
...
<Step title="Implement the Event Handler">
...
<Warning>
Preload optimization makes your handlers run **twice**. So instead of direct RPC calls, we're doing it through `context.effect` - the [Effect API](/HyperIndex/Advanced/effect-api).
</Warning>
...
<Step title="Create the Token Metadata Effect">
...
```typescript icon="/static/icons/typescript-icon.png" title="tokenDetails.ts"
...
<Note>
The `hexToString` method from Viem adds byte padding to the string. We remove this padding with `replace(/\u0000/g, '')` to avoid errors when writing to the database.
</Note>

<Info>
Read more about Effect API and caching in the [Effect API](/HyperIndex/Advanced/effect-api) guide.
</Info>
</Step>
