...
<Note>
Benchmarking adds some memory and processing overhead. It should not be enabled in production environments, as it holds benchmark data points in memory and periodically writes them to disk.
</Note>
...
- If Load Duration is high, consider implementing [Preload Optimization](/HyperIndex/Advanced/preload-optimization)
...
1. **Data Fetching Bottleneck**
   ...
   - **Solutions**:
     - Use [HyperSync](/HyperIndex/Advanced/hypersync) if available for your network
...
2. **Data Loading Bottleneck**
   ...
   - **Solutions**:
     - Implement [Preload Optimization](/HyperIndex/Advanced/preload-optimization) to batch database operations
     - Add appropriate [database indices](/HyperIndex/Advanced/performance/database-performance-optimization) for frequently queried fields
