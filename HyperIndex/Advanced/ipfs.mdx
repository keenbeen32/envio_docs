...
![Basic NFT ownership data](/static/images/docs-assets/ipfs-screenshot.jpg)
...
<Warning>
Preload optimization makes your handlers run **twice**. So instead of direct RPC calls, we're doing it through the [Effect API](/HyperIndex/Advanced/effect-api).
</Warning>

Learn how Preload Optimization works in a [dedicated guide](/HyperIndex/Advanced/preload-optimization). It might be a new mental model for you, but this is what can make indexing thousands of times faster.

Let's create the `getIpfsMetadata` effect in the `src/utils/ipfs.ts` file:

```typescript icon="/static/icons/typescript-icon.png" title="utils/ipfs.ts"
...
![NFT ownership and metadata](/static/images/docs-assets/ipfs-screenshot-2.jpg)
...
Follow the [Effect API Persistence](/HyperIndex/Advanced/effect-api#persistence) guide to implement caching for local development. This should allow you to avoid repeatedly fetching the same data.
...
<Warning>
While the example repository includes SQLite-based caching, this approach is outdated and leads to many indexing issues.
</Warning>

<Note>
We're working on a better integration with the hosted service. Currently, due to the cache size, it's not recommended to commit the `.envio/cache` directory to the GitHub repository.
</Note>

### 4. Learn about Preload Optimization

Learn how Preload Optimization works and the [Double-Run Footgun](/HyperIndex/Advanced/preload-optimization#double-run-footgun) in a [dedicated guide](/HyperIndex/Advanced/preload-optimization). It might be a new mental model for you, but this is what can make indexing thousands of times faster.
