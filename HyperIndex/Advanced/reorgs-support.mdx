...
Reorg detection is guaranteed when using [HyperSync](/HyperIndex/Advanced/hypersync) as your data source. HyperSync's architecture ensures that any reorganization in the blockchain will be properly detected and handled.

### RPC Limitations

When using a [custom RPC endpoint](/HyperIndex/Advanced/rpc-sync) as your data source, there are some edge cases where reorgs might go undetected, depending on the RPC provider's implementation and your indexing pattern.
